...
def get_albu_transforms(type_ = 'train', output_size = (1024, 1024)):
    ...
    assert type_ in ['train', 'test', 'pad', 'resize', "test_extra_noise", "train_heavy"] , "type_ must be 'train' or 'test' of 'pad' "
    trans = None
    if type_ == 'train':
        trans = albu.Compose([
    ...
    # 🔥 2. 我们的强化版训练（抗压、抗旋转、抗裁剪）
    if type_ == 'train_heavy':
        trans = albu.Compose([

            albu.Resize(512, 512),

            # --- 空间几何变换 (模拟用户截图、旋转、翻转) ---
            albu.HorizontalFlip(p=0.5),  # 50% 水平翻转

            # 随即旋转
            albu.OneOf([
                albu.Rotate(limit=(47, 90), border_mode=cv2.BORDER_REFLECT_101, p=1.0),
                albu.Rotate(limit=(113, 135), border_mode=cv2.BORDER_REFLECT_101, p=1.0),
                albu.Rotate(limit=(250, 290), border_mode=cv2.BORDER_REFLECT_101, p=1.0),
                albu.Rotate(limit=(319, 349), border_mode=cv2.BORDER_REFLECT_101, p=1.0),
            ], p=0.45),

            # 随机裁剪并缩放 (模拟用户只截取了图片的一部分)
            albu.RandomResizedCrop(height=output_size[0], width=output_size[1],
            ...
            # 极其核心的 JPEG 压缩 (偏极端，覆盖 50~95 画质)
            albu.ImageCompression(quality_lower=70, quality_upper=95, p=0.6),

            # 随机高斯模糊 (温和，模拟缩放失真)
                albu.GaussianBlur(blur_limit=(3, 7), p=0.35),

            # 随机高斯噪声 (中度，破坏数字指纹)
            albu.GaussNoise(var_limit=(10.0, 30.0), p=0.2),

            albu.OneOf([
                # 旋转 40~50 度，同时伴随 0.6 到 1.2 倍的随机缩放！
                albu.ShiftScaleRotate(shift_limit=0, scale_limit=(-0.4, 0.2), rotate_limit=(40, 50),
                                      border_mode=cv2.BORDER_REFLECT_101, p=1.0),
                albu.ShiftScaleRotate(shift_limit=0, scale_limit=(-0.4, 0.2), rotate_limit=(110, 120),
                                      border_mode=cv2.BORDER_REFLECT_101, p=1.0),
            ], p=0.5),
        ])
