...
class SourceAccuracy(AbstractEvaluator):
    def __init__(self) -> None:
        super().__init__()
        self.name = "source-level Accuracy"
        self.desc = "source-level Accuracy"
        self.correct = torch.tensor(0.0, dtype=torch.float64, device='cuda')
        self.total = torch.tensor(0.0, dtype=torch.float64, device='cuda')
    ...
    def batch_update(self, predict_label, label, *args, **kwargs):
        ...
        if pred is not None and target is not None:
            self.correct += torch.sum((pred == target).float()).to(dtype=torch.float64)
            self.total += torch.tensor(len(target), dtype=torch.float64, device='cuda')

        # 返回 None 意味着不在每个 Iteration 打印，仅在 Epoch 结束时打印
        return None

    def epoch_update(self):
        t = torch.tensor([self.correct, self.total], dtype=torch.float64, device='cuda')
        if dist.is_initialized():
            dist.barrier()
            dist.all_reduce(t, op=dist.ReduceOp.SUM)
