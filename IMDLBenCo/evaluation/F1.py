...
class ImagePrecision(AbstractEvaluator):
    def __init__(self, threshold=0.5) -> None:
        super().__init__()
        self.name = "image-level Precision"
        self.desc = "image-level Precision"
        self.threshold = threshold
        self.TP = 0
        self.FP = 0
...
class ImageRecall(AbstractEvaluator):
    def __init__(self, threshold=0.5) -> None:
        super().__init__()
        self.name = "image-level Recall"
        self.desc = "image-level Recall"
        self.threshold = threshold
        self.TP = 0
        self.FN = 0
...
class PixelPrecision(PixelF1): # 继承PixelF1以复用混淆矩阵计算逻辑
    def __init__(self, threshold=0.5, mode="origin") -> None:
        super().__init__(threshold, mode)
        self.name = "pixel-level Precision"
        self.desc = "pixel-level Precision"
        self.total_TP = torch.tensor(0.0, dtype=torch.float64, device='cuda')
        self.total_FP = torch.tensor(0.0, dtype=torch.float64, device='cuda')

    def Cal_Precision(self, TP, TN, FP, FN):
        return TP / (TP + FP + 1e-8)

    def batch_update(self, predict, mask, shape_mask=None, *args, **kwargs):
        if self.mode == "origin":
            TP, TN, FP, FN = self.Cal_Confusion_Matrix(predict, mask, shape_mask)
            val = self.Cal_Precision(TP, TN, FP, FN)
        elif self.mode == "reverse":
            TP, TN, FP, FN = self.Cal_Reverse_Confusion_Matrix(predict, mask, shape_mask)
            val = self.Cal_Precision(TP, TN, FP, FN)
        elif self.mode == "double":
            TP, TN, FP, FN = self.Cal_Confusion_Matrix(predict, mask, shape_mask)
            val = torch.max(self.Cal_Precision(TP, TN, FP, FN), self.Cal_Precision(FN, FP, TN, TP))
        else:
            raise RuntimeError(f"Cal_Precision no mode name {self.mode}")

            # 累加 Batch 结果
        self.total_TP += TP.sum().to(dtype=torch.float64)
        self.total_FP += FP.sum().to(dtype=torch.float64)

        return None
    #     return val to let it print the val again and again in each batch

    def epoch_update(self):
        t = torch.tensor([self.total_TP, self.total_FP], dtype=torch.float64, device='cuda')
        if dist.is_initialized():
            dist.barrier()
            dist.all_reduce(t, op=dist.ReduceOp.SUM)
        TP = t[0].item()
...
class PixelRecall(PixelF1):
    def __init__(self, threshold=0.5, mode="origin") -> None:
        super().__init__(threshold, mode)
        self.name = "pixel-level Recall"
        self.desc = "pixel-level Recall"
        self.total_TP = torch.tensor(0.0, dtype=torch.float64, device='cuda')
        self.total_FN = torch.tensor(0.0, dtype=torch.float64, device='cuda')

    def Cal_Recall(self, TP, TN, FP, FN):
        return TP / (TP + FN + 1e-8)

    def batch_update(self, predict, mask, shape_mask=None, *args, **kwargs):
        if self.mode == "origin":
            TP, TN, FP, FN = self.Cal_Confusion_Matrix(predict, mask, shape_mask)
            val = self.Cal_Recall(TP, TN, FP, FN)
        elif self.mode == "reverse":
            TP, TN, FP, FN = self.Cal_Reverse_Confusion_Matrix(predict, mask, shape_mask)
            val = self.Cal_Recall(TP, TN, FP, FN)
        elif self.mode == "double":
            TP, TN, FP, FN = self.Cal_Confusion_Matrix(predict, mask, shape_mask)
            val = torch.max(self.Cal_Recall(TP, TN, FP, FN), self.Cal_Recall(FN, FP, TN, TP))
        else:
            raise RuntimeError(f"Cal_Recall no mode name {self.mode}")

        self.total_TP += TP.sum().to(dtype=torch.float64)
        self.total_FN += FN.sum().to(dtype=torch.float64)

        return None
#     return val to let it print the val again and again in each batch

    def epoch_update(self):
        t = torch.tensor([self.total_TP, self.total_FN], dtype=torch.float64, device='cuda')
        if dist.is_initialized():
            dist.barrier()
            dist.all_reduce(t, op=dist.ReduceOp.SUM)
        TP = t[0].item()
...
class SourcePrecision(AbstractEvaluator):
    def __init__(self, num_classes=4) -> None:
        super().__init__()
        self.name = "source-level Precision"
    ...
    def batch_update(self, predict_label, label, *args, **kwargs):
        ...
        return None  # 返回 None，不在每个 iteration 打印
...
class SourceRecall(AbstractEvaluator):
    def __init__(self, num_classes=4) -> None:
        super().__init__()
        self.name = "source-level Recall"
        self.num_classes = num_classes
        self.conf_matrix = torch.zeros((num_classes, num_classes), dtype=torch.float64, device='cuda')
...
class SourceF1(AbstractEvaluator):
    def __init__(self, num_classes=4) -> None:
        super().__init__()
        self.name = "source-level F1"
        self.num_classes = num_classes
        self.conf_matrix = torch.zeros((num_classes, num_classes), dtype=torch.float64, device='cuda')
