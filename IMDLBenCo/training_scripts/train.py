"""
@File: train.py
@Description: 在本项目中没啥用
全部功能迁往 train-AniXplore.py
"""
...
from IMDLBenCo.evaluation import (
    PixelF1, ImageF1, PixelAccuracy, ImageAccuracy,
...
def main(args, model_args):
    ...
    print("=> A complete freeze strategy is being implemented...")
    # 1. 首先，残暴地把所有参数的梯度全部关掉
    for param in model_without_ddp.parameters():
        param.requires_grad = False

    # 2. 接着，把你想要训练的模块（比如 source_head 和 auto_weight）单独打开
    # 假设你只想训练溯源分支
    # for param in model_without_ddp.source_head.parameters():
    #     param.requires_grad = True
    #
    # for param in model_without_ddp.auto_weight.parameters():
    #     param.requires_grad = True

    # 2. 解冻负责伪造检测的模块
    # 解冻分类头（真假二分类）
    for param in model_without_ddp.cls_head.parameters():
        param.requires_grad = True

    # 解冻掩码预测头（预测哪块被改了）
    for param in model_without_ddp.predict_head.parameters():
        param.requires_grad = True

    # 强烈建议解冻特征金字塔和融合层！这样模型才能学会在野图的模糊/压缩下重新组织特征
    for param in model_without_ddp.featurePyramid_net.parameters():
        param.requires_grad = True

    for param in model_without_ddp.fusion_layers.parameters():
        param.requires_grad = True
