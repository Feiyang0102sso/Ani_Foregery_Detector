...
def print_metrics_table(metrics_dict, title="Metrics"):
    ...
    Requires 'tabulate' package, otherwise falls back to simple printing.
    """
    try:
        from tabulate import tabulate
        # Convert dictionary to list of lists for tabulate
        table_data = [[k, f"{v:.4f}" if isinstance(v, float) else v] for k, v in metrics_dict.items()]
        print("\n" + tabulate(table_data, headers=["Metric", "Value"], tablefmt="grid", showindex=False))
    except ImportError:
        print(f"\n===== {title} =====")
