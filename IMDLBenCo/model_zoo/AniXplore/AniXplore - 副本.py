...
class AniXplore(nn.Module):
    def __init__(self, seg_pretrain_path, conv_pretrain=False, freeze_segmodel=False, image_size=512):
        ...
        self.auto_weight = AutomaticWeightedLoss(num=3)
    ...
    def forward(self, image, mask, label, source_label=None, *args, **kwargs):
        ...
        # raw_source_logit = self.source_head(fused_feat)
        ...
        if source_label is not None:
            ...
            source_loss = torch.tensor(0.0, device=image.device)
        ...
        if disable_source_loss:
            # 🚨 野图微调模式：彻底禁用 auto_weight！
            # 强制按 2:1 的比例硬性相加，逼迫模型优先关注局部掩码 (loss)，其次才是全局分类 (cls_loss)
            # combined_loss = 2.0 * loss + 1.0 * cls_loss
        ...
        output_dict = {
            ...
            # 把原始输出传给 UI 计算百分比
            "raw_source_logit": raw_source_logit,
            ...
            "visual_loss": {
                "predict_loss": loss,
                "cls_loss": cls_loss,
                "source_loss": source_loss,  #[新增] 记录溯源 Loss
