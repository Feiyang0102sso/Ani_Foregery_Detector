  ...
  <div class="p-6 flex flex-col gap-6 glass-panel rounded-lg flex-1">
    ...
    <Teleport to="body">
      <div v-if="showModal" class="fixed inset-0 bg-black/80 backdrop-blur-md flex justify-center items-center z-[1000]">
        <div class="bg-[var(--bg-primary)] p-10 w-[90%] max-w-[700px] flex flex-col gap-6 border-[3px] border-[var(--color-cyber-cyan)] shadow-[0_0_40px_rgba(0,255,204,0.15)] rounded-xl">
          <h3 class="text-3xl font-mono font-extrabold text-[var(--color-cyber-cyan)] tracking-wider">{{ $t('data.apiConfigTitle') }}</h3>
          <p class="text-base font-bold text-[var(--text-muted)] leading-relaxed">{{ $t('data.apiConfigDesc') }}</p>
          <input 
            type="text" 
            v-model="tempApiUrl"
            ...
            placeholder="https://your-hf-space-api.hf.space"
            @keyup.enter="saveApi"
          >
          <div class="flex justify-end gap-6 mt-4 items-center">
            <button @click="showModal = false" class="text-base font-bold text-[var(--text-muted)] hover:text-[var(--text-primary)] px-2 uppercase tracking-widest">CANCEL</button>
            <button @click="saveApi" class="bg-[var(--color-cyber-cyan)] text-black font-extrabold font-display px-8 py-4 text-xl tracking-widest hover:bg-white transition-colors rounded-lg uppercase shadow-[0_4px_15px_rgba(0,0,0,0.1)] dark:shadow-none">{{ $t('data.saveConfig') }}</button>
          </div>
        </div>
      </div>
    </Teleport>
...
const currentApiUrl = ref('http://127.0.0.1:7860');
